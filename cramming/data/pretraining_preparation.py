...
def preprocess_dataset(cfg_data, download_path, num_threads=1, max_raw_chunk_size=1e14):
    ...
    print("PREPROCESS PHASE 1: COLLECT DATA")
    use_auth_token="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"
    raw_datasets = []
    for name, details in cfg_data.sources.items():
        ...
        if details.provider == "huggingface":
            ...
            #         name,
            #         use_auth_token=use_auth_token,  # Add the use_auth_token parameter here
            ...
            # if details.max_size:
            #     import sys
            #     raw_dataset = raw_dataset.take(int(details.max_size / sys.getsizeof(str(raw_dataset[0]))))
                print('coucouuuu')
                print(name)
                raw_dataset = datasets.load_from_disk("/globalscratch/ucl/ingi/amfieren/cramming/outputs/data/oscar-data")
