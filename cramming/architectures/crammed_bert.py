...
def construct_crammed_bert(cfg_arch, vocab_size, downstream_classes=None, for_token_classif=False):
    ...
    elif for_token_classif == False:
        model = ScriptableLMForSequenceClassification(config)
    else:
        print("C'EST BON POUR TOKENCLASSIF")
        model = ScriptableLMForTokenClassification(config)
    return model
