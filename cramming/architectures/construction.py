...
def construct_model(cfg_arch, vocab_size, downstream_classes=None, for_token_classif=False):
    model = None
    if cfg_arch.architectures is not None:
        # attempt to solve locally
        if "ScriptableCrammedBERT" in cfg_arch.architectures:
            model = construct_crammed_bert(cfg_arch, vocab_size, downstream_classes, for_token_classif=for_token_classif)
