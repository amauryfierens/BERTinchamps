...
def launch(cfg):
    ...
    dir_path = '/globalscratch/ucl/ingi/amfieren/Datasets/QUAERO/MEDLINE_JSON/'

    train_name = "only_med_train.json"
    val_name = "only_med_dev.json"
    test_name = "only_med_test.json"
    ...
    # Load your tokenizer
    local_checkpoint_folder = os.path.join("/globalscratch/ucl/ingi/amfieren/cramming",cfg.base_dir, cfg.name, "checkpoints") # Load the latest local checkpoint
    ...
    tokenizer = tokenizer = AutoTokenizer.from_pretrained(checkpoint_name)
    ...
    # # Unfreeze the classifier layer (the last layer)
